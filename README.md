...
**macOS**는 리시버가 앱 묶음 `Brevduva.app`으로 설치된다 — 위치는 `~/Library/Application Support/brevduva/`(`BRV_APP_DIR`로 변경 가능)이고, `~/.local/bin/brv`는 그 안의 실행 파일을 가리키는 심볼릭 링크다. 터미널에서 쓰는 명령은 그대로 `brv`다. 앱 묶음인 이유: Developer ID로 서명·공증한 리시버가 시스템 설정의 **로그인 항목 → 앱 백그라운드 활동**과 **개인정보 보호** 목록에 개발자 이름이 아니라 "Brevduva"로 나오게 하려면 macOS의 서비스 등록 API(SMAppService)로 앱 안의 서비스를 등록해야 한다(macOS 13 이상 — 그 아래 버전과 직접 빌드한 단독 실행 파일은 `~/Library/LaunchAgents`의 plist로 등록한다). 갱신은 같은 한 줄이고 앱을 통째로 교체한다. 이전 버전(단독 실행 파일)에서 올라오면 설치기가 부르는 `brv daemon restart`가 서비스 등록을 앱 쪽으로 옮기고 옛 등록 파일을 지운다 — 손으로 할 일은 없다. 제거는 `brv daemon uninstall` 뒤 앱과 링크를 지우면 된다.
...
갱신하면 등록도 따라온다 — 설치기가 부르는 `brv daemon restart`가 버전이 바뀐 뒤 한 번 탐지된 러너 전부에 등록을 다시 쓴다(0.7.1). 이전 버전의 `--binding`이 남은 등록으로 떠도 그 인자는 무시하고 붙는다. Codex의 도구 승인("Always allow")은 다시 쓰기 뒤에도 유지된다(0.7.2).
...
갱신 뒤: 실행 중인 AI 앱·CLI의 로컬 MCP는 **매 업데이트 후 재시작**해야 새 중계기가 뜬다 — 옛 중계기는 리시버가 새 버전으로 재기동한 것을 보면 이유를 남기고 스스로 종료한다(0.7.3). 이전 버전의 Codex Desktop 작업 연결(`brv connect`)은 새 리시버가 기동할 때 거둬져 옛 worker가 멈춘다. 옛 어댑터가 서버에 확정했지만 넘기지 못한 메시지가 기록에 남아 있으면 리시버 로그에 알린다. 바뀐 명령은 [RECEIVING.md](docs/RECEIVING.md#6-이전-버전에서-바뀐-것).
