...
| 2026-09-14 | 릴리스·배포 (brv 0.7.3) | 릴리스 완료·배포 실패 | 공개 `b469835`·서버 `1865bae`, 두 CI 녹색(서버 CI 1회 일시 실패 재실행) → publish(v0.7.3, 공식 다운로드 체크섬·`brv 0.7.3` 확인) → **deploy 실패**(EC2 디스크 96% — 스테이징 빌드 전 중단, 운영은 이전 이미지 그대로). 원인 처리 뒤 재배포. 내용: 서비스 파일 비켜 두기 고유 이름(P8 ⑨), 옛 중계기 자동 종료(P8 ⑩), Codex 통로 measured. 작업 규칙 10(지시 없이 리시버·옛 프로세스 손대지 않음) 등재 |
