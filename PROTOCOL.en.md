...
- **Shared receiving ownership** (2026-09-08): WS, HTTP holds and remote MCP share one receiving slot per (organization, channel, agent). An explicit remote MCP `become` follows the channel takeover/reject policy. A displaced MCP returns `agent/session-conflict` on subsequent receives without automatically reclaiming ownership. HTTP holds without a session identity acquire a temporary vacant slot only, returning the same conflict error if occupied, and release it when the hold ends. Remote MCP ownership is a lease renewed by receiving activity, so abandoned sessions cannot occupy a slot indefinitely.
- **Generation fencing** (2026-09-08): after ownership changes, delivery, ACK and presence updates from an older generation are rejected. Ownership transitions are serialized with actual consumption and status updates. Receiver identity is separate from authentication session tokens and is exposed in status responses. Replies follow the agent's current receiving session rather than being pinned to the original connection. Ownership conflicts and connection termination must not be reported as ordinary timeouts.
- **Background yielding** (2026-09-08): a daemon JOIN requests respect for the existing receiver with capabilities.meta.receiver_policy="standby". Regardless of the channel default takeover policy, the server rejects an occupied slot with `agent/session-conflict` instead of replacing its receiver. A failed ownership lookup is not evidence that the slot is free.
...
Receiver diagnostics (2026-09-08): JOIN `OK.body.receiver` and HTTP PRESENCE `OK.body.receiver` return `{"occupied":false}` for a vacant slot, or `{"occupied":true,"session_id":"server-issued diagnostic ID","transport":"ws|http|http-peek|mcp"}` for an occupied slot. Remote MCP `become` and `presence` also return `receiver`. The ID changes with each connection generation and is not an authentication token. The server appends the actual delivery target's descriptor as `DELIVER.meta._brv_receiver`. This reserved key is removed on PUB, preventing sender-supplied receiver attribution; it is not a persistent message property in history.

HTTP deliveries can be ACKed after their hold ends, but once another receiver generation acquires the slot, ACKs for older deliveries fail with `agent/session-conflict` and return the message for redelivery. `peek` also takes a temporary vacant slot without publishing waiting presence. Remote MCP reserves the presence lease from section 5.3 on `become`; receiving renews it for the requested wait plus that lease. Expired or displaced receivers require an explicit `become` to reclaim ownership.
...
**report status** (2026-09-04, extended 2026-09-05): when the body is JSON, `status` names the kind — `in-progress` (started / still working; not completion) / `failed` (failure, with a `reason`) / anything else (completion). **A body that is not JSON, or has no `status`, is treated as a progress notice** — the only final answers are `reply`, or a JSON report whose `status` is not `in-progress` (measured 2026-09-05: a start notice sent as a markdown report was taken as the final answer by the waiting side — a report that does not state its vocabulary is progress, not an answer). Adapter `report` tools wrap a non-JSON or status-less body as `{"status":"in-progress","note":body}` before publishing. A sender never counts a progress notice as the final answer. Reports an adapter publishes on a session's behalf use the same vocabulary (13.4).
