...
- 러너 등록은 갱신이 스스로 다시 쓴다(0.7.1) — 설치기가 부르는 `brv daemon restart`가 버전이 바뀐 뒤 한 번, 탐지된
  ...
  등록으로 중계기가 떠도 그 인자는 무시하고 붙는다(정체성은 `become`). Codex에서 "Always allow"로 쌓은 도구 승인은
  다시 쓰기 뒤에도 그대로다(0.7.2) — 이미 같은 등록이면 파일을 건드리지 않는다.
- 갱신 뒤 앱 안에 떠 있던 옛 중계기는 리시버가 새 버전으로 재기동한 것을 보면(5초 안에) 스스로 종료한다(0.7.3) — 앱에는
  MCP 서버 종료로 보이고, 그 앱의 MCP를 재시작하면 현재 중계기가 뜬다. 종료 전에 온 요청은 같은 이유로 거부된다.
...
판정·기록·확정·갱신 규칙을 검증했다. 실제 모델 왕복은 윈도우 운영 서비스에서 Claude Code(무인 깨우기·수동 수신·Monitor
밀어넣기 — GUI 앱·CLI)와 Codex(무인 깨우기·codex queue·Codex Desktop)까지 확인했다(2026-09-14). Channels와 macOS·Linux
실기는 아직이다(RECEIVER_REBUILD_PLAN 12단계).
