...
- **The receiver can be operated in plain language, too** — the local MCP server registered in your runners carries `receiver_*` tools (`receiver_status`, `receiver_configure`, `receiver_wake_test`, `receiver_enroll`, `receiver_daemon`, `receiver_binding_remove`, `receiver_mcp_register`). "Switch the backend to codex" is `brv wake set --runner codex` — the tools are thin wrappers that run the CLI itself, so the two cannot drift and the manual commands stay. They exist **only in sessions a human is sitting in**: inside a session the daemon woke (`BREVDUVA_BINDING`), or while a wake is in progress on this machine, they are absent from the tool list and refused if called — so nobody who can message the channel can widen this machine's allowance by asking. Widening to `full` or removing a binding/service returns "ask the owner" unless `confirm=true` is passed
...
Task connection updates: the installer runs `brv connection restart` to restart connected workers in the current profile using the new binary. Paused and disconnected bindings stay unchanged; restart other profiles separately with their `BREVDUVA_CONFIG`. In each running AI app, restart the local MCP **after every update** to load the updated tools. The installer cannot automatically restart app-owned MCP processes and prints this guidance at the end of installation and enrollment. Use the app's MCP restart action, or save your work and fully quit/reopen the app. Relaunch CLI sessions with the same MCP/Channels options. Opening a new chat alone may reuse the existing MCP process; if the app was closed, just start it. Uncertain deliveries are never replayed automatically; follow the [manual recovery procedure (Korean)](docs/DESKTOP_RECEIVER.md#불명확한-전달-복구).

Claude Code channel adapter (starting with 0.6.33): `brv mcp --claude-channel` handles notifications and `receipt` acknowledgements in the MCP process owned by the Claude session. Channels must be enabled at Claude startup and are subject to the allowlist and organization policy. See [setup, recovery and verification scope (Korean)](docs/CLAUDE_CHANNEL.md).
