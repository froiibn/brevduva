...
After updating: restart the local MCP of running AI apps and CLIs **after every update** so the new relay starts — an old relay whose version differs from the receiver says why and exits. A Codex Desktop task connection made with an earlier `brv connect` is retired when the new receiver starts, which stops the old worker. If the old adapter's journal holds messages it confirmed to the server but never handed over, the receiver lists them in its log. For the commands that changed, see [RECEIVING.md (Korean)](docs/RECEIVING.md#6-이전-버전에서-바뀐-것).
