...
| 2026-09-14 | 릴리스·배포 (brv 0.7.1) | 완료 | 공개 `d4253f5`·서버 `af8c887`, 두 CI 녹색 → publish(태그 v0.7.1, 릴리스 워크플로 5종 자산) → deploy(이미지 c03b23d083f4로 server·web 교체). 배포 후 healthz·설치 스크립트·공식 다운로드 확인 |
| 2026-09-14 | Codex 도구 승인 보존 (P8 ⑧, brv 0.7.2) | 완료 | 0.7.1 실기(사용자가 Codex 승인 프롬프트를 직접 넣어 줌): "Always allow"는 `~/.codex/config.toml`의 `[mcp_servers.brevduva.tools.<도구>] approval_mode = "approve"`로 남고, `codex mcp add`가 그 하위 표를 통째로 지운다(격리 실측) → 갱신마다 프롬프트 재발. `codex_registration.rs`: 이미 같은 등록이면 건드리지 않음, 다시 쓸 때 `tools` 표 보존(`toml_edit`, 항목 바로 아래 자리 지정 — 자리를 안 주면 다른 서버 항목 뒤에 흩어짐을 실측). 이 머신 Codex에는 수신·발신 도구 16종 승인을 채움(관리 도구 `receiver_*` 7종은 계속 묻게 둠). 시험 202건 |
| 2026-09-14 | 릴리스·배포 (brv 0.7.2) + 웹·README 문구 대조 | 완료 | 공개 `27756ee`·서버 `fe68271`, 두 CI 녹색 → publish(v0.7.2) → deploy(이미지 3c72e1ce42f2), healthz·공식 다운로드 체크섬·`brv 0.7.2` 확인. 이어서 코드 기준으로 웹(brevduva.dev)·README를 대조: 웹은 MCP 등록 한 줄(`claude mcp add`)·서비스 선택·"토큰은 키체인에만"·옛 버전 표기를 고쳐 서버 커밋 `d255351`로 배포, README는 Codex 설정 예시(`{prompt}` → `-`)·"앱만 열면 설정 불요"·옛 `--env` 문제 해결 항목을 고침(KO/EN) |
| 2026-09-14 | 12. 실기 검증 (3차 — 윈도우 운영 서비스, 실제 앱) | 완료(윈도우) | 사용자 보고 "윈도우에서 코덱스, 클로드 gui, cli 모두 통과"를 리시버 기록·서비스 로그로 대조: Codex CLI(codex queue 7건)·Codex Desktop(4건)·Claude Code GUI/CLI(Monitor 11건) 전달→receipt→reply, 전부 SCM 서비스가 사용자 명의로 전달. `runners.rs` codex queue·Codex Desktop measured, RUNNERS.md·RECEIVER_DESIGN §3·README·RECEIVING 갱신. 남은 실기: Channels·macOS·Linux. 같은 로그에서 0.7.2 갱신이 서비스에 닿지 못한 결함 발견 → 0.7.3(P8 ⑨) |
| 2026-09-14 | 릴리스·배포 (brv 0.7.3) | 완료 (배포는 2026-09-16) | 공개 `b469835`·서버 `1865bae`, 두 CI 녹색(서버 CI 1회 일시 실패 재실행) → publish(v0.7.3, 공식 다운로드 체크섬·`brv 0.7.3` 확인) → **deploy 실패**(EC2 디스크 96% — 스테이징 빌드 전 중단, 운영은 이전 이미지 그대로). **2026-09-16 재배포 완료** — EC2 회수(96%→30%, 원인: buildx 없는 레거시 빌더가 배포마다 약 3.8GB 중간 스테이지를 남김) 뒤 서버 `bbb1c77`(release.py에 여유 공간 검사·빌드 잔재 회수)로 deploy, healthz 200·두 칸 이미지 일치·로그 오류 0. 내용: 서비스 파일 비켜 두기 고유 이름(P8 ⑨), 옛 중계기 자동 종료(P8 ⑩), Codex 통로 measured. 작업 규칙 10(지시 없이 리시버·옛 프로세스 손대지 않음) 등재 |
| 2026-09-22 | macOS 서명·공증 + 앱 묶음(`Brevduva.app`)·SMAppService 등록, 윈도우 exe 아이콘 — 릴리스·배포 (brv 0.7.4) | 완료 | 공개 `a86303c`·서버 `9a1a31b`(핀 + `Cargo.lock`), 두 CI 녹색(공개 3-OS, 서버 test·image) → `release.py check` → publish(v0.7.4: 자산 5종 체크섬 대조, 맥 두 타깃 공증 Accepted·증명 첨부·`Notarized Developer ID`, 맥 자산 내용물 = `Brevduva.app`, 윈도우 exe의 ProductName·아이콘 확인) → deploy(이미지 88548d54ed30, 두 칸 일치, healthz 200/200, 오류 로그 0, `brevduva.dev/install.sh`가 릴리스 커밋의 것과 해시 일치). 맥 실기는 2026-09-21~22(설계 이력 RECEIVER_DESIGN P8). 알려 둘 것: publish와 deploy 사이에는 옛 `install.sh`가 새 맥 자산을 다루지 못해 맥 설치가 (아무것도 바꾸기 전에) 실패한다 — 이번에는 약 1분 20초(공개 16:35:26Z → 새 웹 칸 기동 16:36:48Z).자산의 모양을 바꾸는 릴리스는 새 스크립트가 옛 자산도 다루게 해 두었으므로, 다음부터는 스크립트를 먼저 내보내는 절차를 검토한다 |
